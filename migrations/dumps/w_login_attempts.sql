DROP TABLE IF EXISTS `w_login_attempts`;

CREATE TABLE `w_login_attempts` (
  `w_login_attempt_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Wログイン失敗ID',
  `ip` varchar(45) NOT NULL COMMENT 'ipアドレス(IPv4, IPv6)',
  `login` varchar(128) NOT NULL COMMENT 'ログインID(login, email)',
  `status` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT 'ログインステータス [0:ログイン失敗 1:ログイン成功]',
  `session_id` varchar(128) NOT NULL DEFAULT '' COMMENT 'ログイン成功時のセッションID(重複ログインチェック用)',
  ...
  PRIMARY KEY (`w_login_attempt_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Wログイン失敗';
