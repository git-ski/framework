...
use Project\Inquiry\Admin\Fieldset\InquiryListFieldset;
use Project\Viewpack\Material\ViewModel\PaginatorViewModel;
use Project\Inquiry\Admin\Controller\Inquiry\ListController;
...
{
    protected $template = '/template/inquiry/list.twig';
    ...
    public function onRender(): void
    {
        ...
        $PaginatorViewModel->setData([
            'paginator' => $data['inquiry']->getPages(),
        ]);

        $form = $this->getForm();
        $form->submit->setValue('検索');
        if (isset($data['keyword'])) {
            $form->inquiry->keyword->setValue($data['keyword']);
        }
        $items = [];
        foreach ($data['inquiry'] as $Inquiry) {
            $items[] = $Inquiry->toArray();
        }
