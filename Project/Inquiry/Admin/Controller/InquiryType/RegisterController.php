...
use Project\Inquiry\Admin\View\ViewModel\InquiryType\RegisterViewModel;
use Project\Inquiry\Admin\Controller\InquiryType\RegisterModel as InquiryTypeRegisterModel;
...
class RegisterController extends AbstractAdminController
{
    ...
    public function onRegisterFinish(\Framework\EventManager\Event $event): void
    {
        ...
        $InquiryTypeRegisterModel = $this->getObjectManager()->get(InquiryTypeRegisterModel::class);
        $inquiryTypeEntity = $InquiryTypeRegisterModel->create($inquiryType);
        $this->getObjectManager()->get(FlashMessage::class)->add('FlashMessage', 'Created');
        $this->triggerEvent(self::TRIGGER_AFTER_CREATE, $inquiryTypeEntity);
    ...
    {
        return [
            "title"             => "お問合せタイプ登録", // title
            'description'       => "お問合せタイプ登録",
