...
use Project\Inquiry\Admin\View\ViewModel\InquiryType\EditViewModel;
use Project\Inquiry\Admin\Controller\InquiryType\EditModel as InquiryTypeEditModel;
...
class EditController extends AbstractAdminController
{
    ...
    {
        $InquiryTypeEditModel = $this->getObjectManager()->get(InquiryTypeEditModel::class);
        $InquiryType = $InquiryTypeEditModel->get($id);
        if (!$InquiryType) {
    ...
    public function onEditFinish(\Framework\EventManager\Event $event): void
    {
        ...
        $inquiryType = $ViewModel->getForm()->getData()['inquiryType'];
        $InquiryTypeEditModel = $this->getObjectManager()->get(InquiryTypeEditModel::class);
        $InquiryType = $InquiryTypeEditModel->get($inquiryType['inquiryTypeId']);
        $InquiryTypeEditModel->update($InquiryType, $inquiryType);
        $this->getObjectManager()->get(FlashMessage::class)->add('FlashMessage', 'Updated');
        $this->triggerEvent(self::TRIGGER_AFTER_UPDATE, $InquiryType);
        $this->getRouter()->redirect(ListController::class);
    }
