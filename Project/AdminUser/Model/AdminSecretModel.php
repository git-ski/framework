...
use Project\AdminUser\Entity\AdminSecret;
use Project\AdminUser\Model\AdminModel;
use InvalidArgumentException;

class AdminSecretModel extends AbstractEntityModel implements
    ...
    CryptManagerAwareInterface
{
    ...
    public function create($data) : EntityInterface
    {
        ...
        $AdminSecret->fromArray($data);
        $DateTime = $this->getDateTimeForEntity();
        $this->getEntityManager()->persist($AdminSecret);
     ...
     */
    public function get($idOrAdminSecret)
    {
        if ($idOrAdminSecret instanceof AdminSecret) {
            return $idOrAdminSecret;
        }

        return $this->getRepository()->findOneBy([
            'adminSecretId' => $idOrAdminSecret,
        ]);
    }
     ...
     */
    public function update($idOrAdminSecret, $data = null) : EntityInterface
    {
        if (!$idOrAdminSecret instanceof EntityInterface) {
            $idOrAdminSecret = $this->get($idOrAdminSecret);
        }
        if (!$idOrAdminSecret instanceof EntityInterface) {
            throw new InvalidArgumentException('Invalid Entity OR Id For Update: AdminSecret');
        }
        if ($data) {
            $data = $this->filterValues($data);
            if (isset($data['Admin'])) {
            ...
            $idOrAdminSecret->fromArray($data);
        }
        $DateTime = $this->getDateTimeForEntity();
        $this->getEntityManager()->merge($idOrAdminSecret);
        $this->getEntityManager()->flush();
        return $idOrAdminSecret;
    }
     ...
     * @throws InvalidArgumentException 更新対象が見つからない場合はExceptionを投げる
     */
    public function delete($idOrAdminSecret) : EntityInterface
    {
        throw new InvalidArgumentException('AdminSecret にdeleteFlagが存在しないため、論理削除できません。');
        $AdminSecret = $this->get($idOrAdminSecret);
        if (!$AdminSecret instanceof EntityInterface) {
            throw new InvalidArgumentException('Invalid Entity OR Id For Delete: AdminSecret');
        }
        $AdminSecret->setDeleteFlag(true);
        $this->getEntityManager()->merge($AdminSecret);
        $this->getEntityManager()->flush();
        return $AdminSecret;
    }
     ...
     * @return void
     */
    public function remove($idOrAdminSecret)
    {
        $AdminSecret = $this->get($idOrAdminSecret);
        if (!$AdminSecret instanceof EntityInterface) {
            throw new InvalidArgumentException('Invalid Entity OR Id For Delete: AdminSecret');
        }
        $this->getEntityManager()->remove($AdminSecret);
        $this->getEntityManager()->flush();
    }
    ...
    public function getRepository()
    {
        if (null === $this->Repository) {
            $this->Repository = $this->getEntityManager()->getRepository(AdminSecret::class);
        }
    ...
    {
        $propertyLabels = [
            'adminSecretId' => 'ADMINSECRET_ADMINSECRETID',
            'secret' => 'ADMINSECRET_SECRET',
            'code' => 'ADMINSECRET_CODE',
            'Admin' => 'ADMINSECRET_ADMIN',
        ];
    ...
    {
        // 主キーフィルター
        if (isset($data['adminSecretId'])) {
            unset($data['adminSecretId']);
        }
        // 空白文字列フィルター
